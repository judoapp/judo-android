...
internal class Interpolator(
    ...
    fun interpolate(theTextToInterpolate: String): String? {
        return try {
            interpolateWithExceptions(theTextToInterpolate)
        } catch (exception: Exception) {
            if (exception is StringExpressionException || exception is java.lang.NumberFormatException) {
                ...
                android.util.Log.e(
                    app.judo.sdk.compose.ui.data.Interpolator.Companion.TAG,
     ...
     */
    internal fun interpolateWithExceptions(theTextToInterpolate: String): String {
    ...
    private fun dateFormatHelper(arguments: List<String>): String {
        ...
        try {
            ...
            Log.e(
                TAG,
                "Failed to format the given date: $dateString, with the given format: ${arguments[2]}. Returning unformatted $dateString."
            )
            return dateString
        }
    }
    ...
    private fun tryParsingDateOrDateTime(input: String): TemporalAccessor {
        ...
        for (type in dateTimeFormatterTypes) {
            try {
                ...
                Log.d(
                    TAG,
                    "parseDateOrDateTime couldn't parse $input with $type. Skipping."
                )
                continue
            }
        }
...
internal sealed class StringExpressionException(
    ...
    class UnexpectedValue(message: String = "Unexpected value (or missing data)", value: String) :
