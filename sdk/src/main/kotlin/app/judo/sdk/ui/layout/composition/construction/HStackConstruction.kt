...
internal suspend fun HStack.construct(
    ...
    val frameView = ExperienceView(context, resolvers = resolvers).apply {
    ...
    val constructedViews = listOfNotNull(background, frameView) + childViews + listOfNotNull(overlay)
    childViews.forEach { it.alpha = it.alpha * (opacity ?: 1f) }
    frameView.alpha = frameView.alpha * (opacity ?: 1f)
