...
internal class IntegerParser<U> : AbstractParser<U, Int>() {
    ...
    override fun parseNonEmptyInput(input: ParserContext<U>): Parser.Result<U, Int> {
        return try {

            MapParser(AndParser(maybeAMinusParser, oneOrMoreNumbersParser)) { (minus, strings) ->
                "${minus?.toString() ?: ""}$strings".toInt()
            }.parse(input)
